...
class Sober_luna16(object):
    def __init__(self, workspace):
        ...
        self.all_patients_path = os.path.join(self.workspace,"data/mhd_train/")
        self.jpg = os.path.join(self.workspace, "data/jpg/")
        self.npy_node_train = os.path.join(self.workspace, "data/npy_cube_train")
        self.npy_node_result = os.path.join(self.workspace, "data/npy_cube_result")
        self.ls_all_patients = glob(self.all_patients_path + "*.mhd")
        print(self.ls_all_patients)
        self.df_annotations = pd.read_csv(self.workspace + "data/csv/train/annotations.csv")
        ...
        for fcount, img_file in enumerate(tqdm(self.ls_all_patients)):
            ...
            if mini_df.shape[0] > 0:  # 有些病人可能没有结节，跳过这些病人some files may not have a nodule--skipping those
                ...
                for node_idx, cur_row in mini_df.iterrows():
                    ...
                    np.save(os.path.join(self.npy_node_train, "%04d_%04d_%04d_%s.npy" % (fcount, node_idx, i_z, cur_row["seriesuid"])), slice)
                    np.save(os.path.join(self.npy_node_result, "%04d_%04d_%04d_%s_o.npy" % (fcount, node_idx, i_z, cur_row["seriesuid"])), nodule_mask)
                    #===================================
                    #---以下代码是生成图片来观察分割是否有问题的
                    nodule_mask = 255.0 *nodule_mask
                    nodule_mask = nodule_mask.astype(np.uint8)
                    #cv2.imwrite(os.path.join(self.npy_node_train, "jpg_%04d_%04d_%04d_%s.jpg" % (fcount, node_idx,i_z,cur_row["seriesuid"])),slice)
                    #cv2.imwrite(os.path.join(self.npy_node_result, "jpg_%04d_%04d_%04d_%s_o.jpg" % (fcount, node_idx,i_z,cur_row["seriesuid"])),nodule_mask)
